...
/* Signing our message, with no hashing. We want anyone with public key to read contents. */
    ...
    if( ( ret = mbedtls_pk_sign( &pk_private, MBEDTLS_MD_SHA1 , message, strlen(message), sign, &olen,
...
/* Verification using our public key */
	...
	//fflush( stdout );
	mbedtls_md_info_t *md_info = mbedtls_md_info_from_type(MBEDTLS_MD_SHA1);
	const char md[32];
	mbedtls_md(md_info,signed_base64_decoded,strlen(signed_base64_decoded),md);
	PRINTF("hash value: %s /n:",md);
	//Isn't this size too big for RSA2048?
	if((ret = mbedtls_pk_verify(&pk_public,MBEDTLS_MD_SHA1,md,32,signed_base64_decoded,strlen(signed_base64_decoded))) != 0)
	{
